...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Question text scroll animations - show one by one with delay
    const questionTexts = document.querySelectorAll('.question-text');
    let questionsShown = false;

    const questionObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
            if (entry.isIntersecting && !questionsShown) {
                questionsShown = true;
                // Show first question
                questionTexts[0].classList.add('visible');
                
                // Show second question after delay
                if (questionTexts[1]) {
                    setTimeout(() => {
                        questionTexts[1].classList.add('visible');
                    }, 800);
                }
            }
        });
    ...
    if (questionTexts.length > 0) {
        questionObserver.observe(questionTexts[0]);
    }
